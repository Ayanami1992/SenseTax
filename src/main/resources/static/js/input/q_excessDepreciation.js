var vm = new Vue({
	...
	methods: {
		init() {
			this.$http.post('./q_init?_csrf='+$("#_csrf").val(), this.form).then(function(data) {
				App.unblock();
		...
		onSubmit(evt) {
			evt.preventDefault()
			this.$http.post('./q_insert?_csrf='+$("#_csrf").val(), this.form).then(function(data) {
				 console.log(data);
				 $("#confirmClose").click();
